...
func NewConfigFile(path string) (*ConfigFile, error) {
	...
	if err != nil {
		slog.Warn("Error while statting config file", "error", err)
		return nil, fmt.Errorf("error while statting config file %s: %w", path, err)
	}
	if fileInfo.IsDir() {
		slog.Warn("Config file is a directory", "path", path)
...
func (c *ConfigFile) Load(target any) error {
	...
	if err := c.viper.ReadInConfig(); err != nil {
		slog.Warn("Error while reading config file", "error", err)
		return fmt.Errorf("error while reading config file %s: %w", c.Path, err)
	}

	if err := c.viper.Unmarshal(target); err != nil {
		slog.Warn("Error while unmarshalling config file", "error", err)
